...
CRAN (Comprehensive R Archive Network) is a central location where fully-vetted R packages are shared with the world, but you can just as easily create and share packages in a shared folder, on GitHub, or elsewhere. For example, many R packages are hosted on GitHub, so if you want to install them, you do not need to get them from CRAN. You can go straight to the source on GitHub and download it like so:
...
Running the function `devtools::document()` automatically creates all of the help pages in the folder `man` along with a list of the package functions in the `NAMESPACE` file. The only other files that are needed for the package are the `DESCRIPTION` and `LICENSE`.
