...
## The new select_*() and rename_*() functions
...
Another example are the functions `%m+%` and `%m-%`. This will add months to a date 
without creating a date that doesn't make sense.
...
Another product of **lubridate** is the object type `interval`. You can create one 
